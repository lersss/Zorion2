...
14. **Тестировщик `@tester`** (модель `deepseek-v4-flash`, `.opencode/agents/tester.md`). Нагружай его когда нужен независимый прогон; прогоны он отмечает в чек-листе `docs/QA_CHECKLIST.md` (Ядро + Зоны, перезапись дат). Это не шлюз — вердикт можно не ждать.
