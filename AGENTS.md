...
15. **Менеджер `@manager`** (модель `big-pickle`, `.opencode/agents/manager.md`). Приёмная пожеланий: наговорил фичу/баг — менеджер уточнит через вопросы, запишет во входящие `docs/gamedesign/ideas/` и поведёт по этапам (дизайн → разработка → тест) **по гейтам**: после каждого этапа ждёт явного одобрения игрока и не продолжает без него. «Дозревшее до дела» переносится в спеку `docs/specs/` и строку в `99_roadmap.md` §99.2 (см. §99.6).
