...
24. **Графический дизайнер `@gdesigner`** (модель `opencode/mimo-v2.5-free`, `.opencode/agents/gdesigner.md`). Визуальный дизайн игры: стилистика, палитры, формы, конфиги генераторов визуала, ТЗ на ассеты и улучшение вида. На старте визуальной задачи ПРЕДЛАГАЕТ 2–4 варианта стилистики — создатель выбирает. Работает по гейтам, код не пишет, не коммитит (вызывается через @manager).
25. **Художник `@artist`** (модель `opencode/deepseek-v4-flash`, `.opencode/agents/artist.md`). Рисует спрайты любых сущностей (корабли, расы, звёзды, планеты и т.д.) через ComfyUI по `docs/gamedesign/art/` (принципы-силуэты, двухэтапный процесс, пост-обработка, рецепты); работает по гейтам отбора создателя, код не пишет, не коммитит (вызывается через @manager).
26. **Оптимизатор `@optimizer`** (модель `opencode/deepseek-v4-flash`, `.opencode/agents/optimizer.md`). Аналитик-оптимизатор промтов и структуры: регулярно сверяет промты агентов со структурой проекта (доки, зоны, правила, реальные файлы) и предлагает дословные дельты для ускорения работы; сам не правит и не коммитит (вызывается через @manager, идея 89a).
27. **Impact map (`docs/impact_map.json`, реестр каскадных влияний, ведёт @manager, идея 83a).** Перед правкой сущности из реестра — сверься с ним: «меняем X → смотрим на Y».
