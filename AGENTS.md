...
1. **Баги всегда приоритетнее фич.** Сначала чинить баги, потом делать новые функции.
2. **Не менять файлы без подтверждения.** Предлагать решение → ждать одобрения → писать код.
3. **Один вопрос за раз.** Не перегружать пользователя несколькими вопросами.
4. **Полные перечни файлов.** При замене файла — давать полный список.
5. **Следовать стилю кода.** Смотреть на соседние файлы, использовать существующие библиотеки.
6. **Безопасность.** Не коммитить секреты, не логировать ключи.
7. **Комментарии только по запросу.** Не добавлять `//` комментарии без просьбы.
8. **Тесты.** Если есть — запускать. Если нет — проверять `go vet` и `go build`.
9. **Одна правка — один коммит.** Не смешивать разные изменения.
10. **Имена файлов в разных папках не должны совпадать.** Исключение — стандартные (`main.go`, `go.mod`, `index.html`).
