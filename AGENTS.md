...
> Подгружается автоматически. Дополнительно на старте: главный агент — `STATUS.md`; субагенты — `.opencode/context.md` (заменяет `STATUS.md`/`docs/INDEX.md` целиком). Больше ничего не читай, пока не потребует задача.
...
- `CHANGELOG.md` — ведёт `@dispatcher` по дельтам менеджеров (раздел „Разнос по докам“); обычным агентам при работе не нужен. Формат записи — в `.opencode/agents/dispatcher.md`.