...
> Незнакомый термин — `docs/GLOSSARY.md`, туда же за разбором многозначных слов.
...
2. **Правки файлов — только по явному «го».**
   ...
   - Директива («сделай X»), вопрос, описание проблемы, молчание — не «го» на детали. Сначала предложение → «го» → код.
   - Один «го» покрывает только предложенный объём; всё сверх него — снова предложение и «го».
   - **Сомневаешься, есть ли «го», — его нет.** Код в состоянии сомнения не пишется.
   - **Не понимаешь, зачем это делаешь, — не делай, а спроси.** Цель должна быть ясна до кода.
   - **Есть решение лучше — предложи его, хотя бы один раз.** Возражение — обязанность, а не любезность: задача агента — расширять картину мира игрока лучшими возможностями, а не вести себя соглашательски.
...
13. **Сервер перезапускать самому.** После правок рантайма (Go-код, миграции, вступающие в силу на старте) — перезапустить локальный dev-сервер и проверить `/health`. Статика (HTML/JS/CSS) отдаётся с диска с `noCache` — рестарт не нужен, но и не вреден.
...
| `docs/GLOSSARY.md` | **Встретил незнакомый термин — сюда.** Один термин — одно определение. §1 разбирает многозначные слова: «пол», «потолок», «фронт», «присутствие», «тир», «тик» |
| `docs/AGENT_RULES.md` | **Открывать не нужно** — подгружается автоматически (`opencode.json`) |
| `docs/NEXT_SESSION.md` | Повестка следующей дизайн-сессии: где встали, что дообсуждать |
...
| `docs/gamedesign/13_tiers.md` | Модель поселения: население и тир, потолки, нагрузка, корзина, кошелёк, естественный запас |
| `docs/gamedesign/13_tiers_impl.md` | Имплементация модели поселения: схема, формулы, пересчёт, тесты. Открывать, когда пишешь код поселений |
...
- **Температура: в данных — Кельвины, в UI — везде °C.** `planet.data.temperature` и Go-модели хранят K. В UI температура отображается только в °C (хелпер `kelvinToCelsius` в `web/static/js/modal/tabs.js`; в форме поселений конверсия `K = °C + 273` в `web/static/js/admin/generation.js`, `buildSettlementModel`). Значение в K — максимум как уточнение в скобках, но не в приоритете.
