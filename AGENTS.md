...
- **Две таблицы ресурсов, живая — не та, что в коде.** `planet_resources` (куда пишет текущий генератор) **пуста**, а legacy-таблица `resources` содержит ~2.6M строк от прежней генерации — именно она описывает текущую вселенную. В Go `INSERT INTO resources` больше нет. Не чини пустую таблицу, не зная про вторую; разруливать одной миграцией при переходе на новую ресурсную модель (`docs/gamedesign/99_roadmap.md` §99.2.5).
