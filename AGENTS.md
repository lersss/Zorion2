...
Поднять все локальные сервера разом — `start-all.cmd` в корне проекта (двойной клик); подробности и переключатели состава — `docs/DEPLOY.md` §2.

`JWT_SECRET` обязателен — без него `log.Fatal`. **`JWT_SECRET` вручную НЕ задавать** — берётся из `.env` в корне проекта
(godotenv). Заданный вручную секрет перекрывает `.env` и
...
- `CHANGELOG.md` — ведёт `@dispatcher` по дельтам менеджеров (раздел «Разнос по докам»); обычным агентам при работе не нужен. Формат — **один раздел `## ДД-ММ-ГГГГ` на дату** (группировка по датам, новые сверху; записи одного дня сливаются в раздел даты); внутри даты — **тематические подзаголовки `### <Тема>` (до 4 штук) + `### Прочее`** (темы групп — по смыслу дня, не обязаны повторяться), а блок `**Исправлено:**` идёт после всех групп одним общим блоком (отдельного блока «Дополнительно» нет — его пункты растворяются в «Прочее» или в подходящей группе); и **понятный для людей язык** (что изменилось для игрока, без хешей/миграций/имён файлов) — правила создателя 2026-09-22 и 2026-09-23.