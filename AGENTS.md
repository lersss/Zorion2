...
18. **Сторож зацикливания и живой учёт агентов.** `.opencode/plugins/loop-guard.js` режет повторный вызов с неизменным результатом (5-й раз), напоминает агенту про исчерпанный бюджет (память ≥ 700k, файл переписан 25 раз, семейство команд 15 раз, три одинаковых вывода подряд), тормозит и гасит сессию (память ≥ 900k); журнал — `.opencode/loop-guard.log`; расходы и память агентов — локальный сервис `tools/agent-dash` (запуск `tools/agent-dash/start.ps1`, http://127.0.0.1:8790).
