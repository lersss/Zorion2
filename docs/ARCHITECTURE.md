...
| Расовый реестр кораблей (спека `2026-09-23-корабли-рас-раса-агентов-и-игрока`, П1) | `internal/models/race_ship_sprites.go` — `RaceShipSprites` (П1: 12 людских 4 типа × 3 варианта, порядок фиксирован, +106 расовых в П3), `NeutralShip`, `DefaultHumanShip`, `ShipOptions` (транспорт `/me.ship_options`), индексы `shipSpriteByFile`/`shipFilesByRace`, `RandomShipForRace`; `ResolveShipIcon`/`IsValidShipIcon`/`ShipOrientByFile` (`ship_sprites.go`) переключены на расовый реестр; колонки `npc_agents.race_id`/`users.race_id` + DEFAULT `users.ship_icon` — миграция `000074`; клиентский индекс «раса → файлы» — `web/static/js/map/ship_sprites.js` (`setShipOptions`, `shipFilesForRace`); селектор дашборда фильтрует пул расы игрока — `web/index.html` |
