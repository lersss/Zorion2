...
| NPC-агенты (спека `20a.1`) | `internal/models/npc_agent.go`, `internal/repository/npc_repository.go` (этап 1: модель, курсорные batch-выборки, Insert/Delete/GetByID/Update); `internal/npc/` (этап 2: `manager.go` — планировщик, `agent_cache.go` — in-memory кэш агентов для позиций (идея 26c A2: один ListAll при старте/инвалидации, инкремент стартами/прибытиями тика, dirty-флаг от внешних мутаций), `worldgrid.go` — выбор маршрута по сетке миров, `position_cache.go` — snapshot позиций, `settings.go` — лимиты §2.4; этап 5: `notification_batch.go` — буфер прибытий с дросселем §2.2.C); ручки (этап 4): `internal/handlers/admin_npc.go` (CRUD), `npc_settings.go` (настройки менеджера), `npc_positions.go` (позиции для карты); уведомления (этап 5): `internal/handlers/ws_notifier.go` (batch → `WSHub.Broadcast`), `websocket_hub.go` (`Broadcast`). Пул «раса → родной мир» (спека `2026-09-23-корабли-рас…` §5, П2): `RaceHomeworldSource` + `RandomRaceHomeworlds`/`RefreshRaceHomeworlds` (`manager.go`, снимок read-only, atomic pointer; инвалидация — по смене снапшота карты и из `GenerateFactions`), SQL — `NPCRepository.RaceHomeworlds()`; генерация агентов ставит расу и стартует в её мире (`admin_npc.go`); одиночное создание (`POST /admin/npc`) тоже берёт расу из пула (пул пуст → 400); `race_id` агента едет в `GET /api/npc/positions` (`InterpolatedPosition.RaceID`) — клиент выбирает расовый корабль |
| JWT + роли | `internal/auth/jwt.go`, `internal/auth/middleware.go`, `internal/auth/admin_auth.go` |
| Визуал кораблей: резолверы и палитра (спека `61b`; легаси выпилен — спека `2026-09-23-корабли-рас-раса-агентов-и-игрока` §8, П4) | `internal/models/ship_sprites.go` — общие определения: тип `ShipSprite`, `ShipColorPalette` (9 + «Оригинал»), резолверы `ResolveShipIcon`/`IsValidShipIcon`/`ShipRaceByFile`/`ShipOrientByFile` на расовом реестре; сам реестр — `race_ship_sprites.go` (строка ниже); клиент — `web/static/js/map/ship_sprites.js` (перекраска hue+destination-in, ленивый прелоадер корабля игрока), `map_render.js`/`npc_agents.js` (рендер); PNG — `web/static/sprites/race_*.png` (117) + `neutral.png`; легаси-21 PNG, `ShipSprites`, `LegacyShipIconMap`, `DefaultShipIcon` удалены (П4); `PUT /me/ship-color` (`internal/handlers/user_handlers.go`) |
| Расовый реестр кораблей (спека `2026-09-23-корабли-рас-раса-агентов-и-игрока`, П1/П3) | `internal/models/race_ship_sprites.go` — `RaceShipSprites` (П3: **117** — 12 людских 4 типа × 3 варианта первыми, порядок фиксирован, + 105 расовых в порядке `ships_meta.json` в конец; ориентация из меты при `orient_meta:true`), `NeutralShip`, `DefaultHumanShip`, `ShipOptions` (транспорт `/me.ship_options`, 118 = 117 + neutral), индексы `shipSpriteByFile`/`shipFilesByRace`, `RandomShipForRace`; `ResolveShipIcon`/`IsValidShipIcon`/`ShipOrientByFile`/`ShipRaceByFile` (`ship_sprites.go`) на расовом реестре; колонки `npc_agents.race_id`/`users.race_id` + DEFAULT `users.ship_icon` — миграция `000074`; клиент — `web/static/js/map/ship_sprites.js` (`setShipOptions`, `shipFilesForRace`, `spriteForAgent(race_id, id)` + `pickVariant` по слоту, ленивый прелоад только корабля игрока), `map/npc_agents.js` (передаёт `p.race_id`); `race_id` агента едет в `GET /api/npc/positions` (`npc/position_cache.go`, `repository/npc_repository.go`); `PUT /me/ship-icon` принимает только файл расы игрока (`auth_handlers.go`); селектор дашборда фильтрует пул расы игрока — `web/index.html`; PNG — `web/static/sprites/race_*.png` (117) |
...
| Арт-студия (67a.1, `cmd/art-studio`, порт 8798) | Вкладки: Отбор · Эталон · Люди · Корабли рас · В игре · Галерея; «В игре» — витрина игрового реестра кораблей, только чтение; эндпоинты `/ships/ingame`, `/ships/ingame/img/<file>` (2026-09-25) |
| Студия товаров (99a.1, перенесена в игровой сервер) | Студия — часть `cmd/server` (перенос A/B/C завершён 2026-09-20): `internal/goodsstudio/` (модель/граф/валидаторы/сид + `ai/` — ИИ-заполнение, env `OPENCODE_URL/MODEL/AGENT/TIMEOUT_S/MAX_RETRIES` + `aiserve/` — управление локальным ИИ-помощником, env `OPENCODE_START_TIMEOUT_S`), роуты `/studio/api/*` и `/studio/api/ai/*` (JWT admin/skycomposer), UI `web/studio.html` (статика с диска, noCache), общий auth `web/static/js/auth.js` (ядро + адаптеры `admin/auth.js`/`studio/auth.js`); `cmd/goods-studio` удалён (2026-09-20), `goods_data/`/`config/goods/` не существуют |
...
| Студия товаров — UI-модули (эпик «Студия: рефакторинг монолита по блокам», 2026-09-25) | `web/studio.html` — только разметка и подключение скриптов; логика — классические скрипты `web/static/js/studio/*.js`: `core.js` (ядро: `state`/`$`/`fetchState`/`esc`/`codeNote`), `content_transfer.js`, `interactions.js` (контекстное меню, drag&drop), `graph.js` (граф товаров), `goods_popup.js`, `fill_ai.js`, `dialogs.js`, `producers_ref.js`, `unit_scale_calc.js`, `items_ref.js`, `buildings_tree.js`, `selectors.js`, `catalog_popups.js`, `init.js` (оверлеи/режимы/разделы, проводка событий и инициализация) + модульные `unit_scale.js`/`auth.js`; каждый классический — со `"use strict";`, порядок подключения — в HTML; инлайнового `<script>` нет (эпик завершён: монолит разнесён на 14 классических модулей) |
| Фабрики — каталог производителей/предметов (спека `2026-09-20-фабрики-сущность-производства`, релиз 1) | Ветки студии «Производители»/«Предметы»: таблицы `producer_types`/`items`/`producer_items` (миграция `000048`, BIGSERIAL как 000045) + дельта `goods.volume/weight` (3b.6.4); сид — `internal/goodsstudio/seed_producers.go` (8 типов + 4 предмета + 4 связи, маркер `producer_catalog_seed`, после `goodsstudio.Seed` — категории уже посеяны); SQL — `internal/repository/producer_repository.go` (CRUD на том же `pg_advisory_xact_lock`); HTTP — `internal/handlers/studio_handlers.go` (+`/studio/api/producers*`, `/studio/api/items*`); UI — `web/studio.html` (разделы Товары/Предметы + Постройки — разделы по видам, поле `producer_types.section`). Снос легаси `production_units` — миграция `000050`, удалён `internal/repository/production_unit_repository.go`, снят из `truncateTables` (`admin_universe.go`) |
...
| Постройка структур на планете (спека `2026-09-24-постройка-структур-на-планете`, ЧК1) | Админ-инструмент «Построить структуру»: `internal/handlers/admin_structures.go` (+`admin_structures_options.go` — сборка данных формы, `admin_structures_create.go` — создание) — `GET /admin/planets/{id}/build-options`, `GET /admin/owner-candidates`, `POST /admin/planets/{id}/structures` (роут — `HandlePlanetRoute`); владелец новым поселениям при генерации — `EnsureSettlementOwners` (`internal/generator/faction/faction.go`); миграция `000082` (`settlements.owner_type/owner_id`, `buildings.producer_type_id`). Клиент — `web/static/js/modal/build.js` (панель «Построить»), `modal/structures.js` (вкладка «Строения») |
...
| Управление локальным ИИ-помощником студии (спека `2026-09-24-студия-управление-локальным-ии`) | Пакет `internal/goodsstudio/aiserve` — состояние `opencode serve` по ФАКТУ пробы `OPENCODE_URL` (TCP + `GET /global/health`, фолбэк `/config`), запуск (`cmd.exe /c npx -y opencode-ai serve --port <порт>`, `DETACHED_PROCESS`, лог `logs/opencode_serve.log`) и гашение по владельцу порта/процессу (`taskkill /PID /T /F`); гейт `managed` (Windows + loopback + `npx`), на проде — «недоступно»; клиент `internal/goodsstudio/ai` не менялся. HTTP — `internal/handlers/studio_ai.go` (`GET /studio/api/ai/status`, `POST /studio/api/ai/start\|stop`, JWT admin/skycomposer), поле `ai` в `StateView` (`studio_handlers.go`); env `OPENCODE_START_TIMEOUT_S` (дефолт 300) — `internal/config/config.go`. UI — `web/studio.html` (`#aiStatus`/`#btnAiStart`/`#btnAiStop` в шапке, дизейбл ИИ-кнопок при `state≠running`); `start-all.cmd` — сервис `[7/7]` порт 3456 + переключатель `AI`; e2e E1–E5 — `tools/e2e/goods-studio-server-check.js` |
| Метка переноса каталога (`code`, миграция `000084`, И1; спека `2026-09-24-каталог-экспорт-импорт-контента-на-прод` §3) | Переносимая идентичность записи каталога у `goods`/`producer_types`/`items`/`effect_types`: колонка `code TEXT NULL` + `SEQUENCE` `<таблица>_code_seq` + DEFAULT `catalog_next_code('<префикс>', '<seq>')` + частичный UNIQUE `(code) WHERE code IS NOT NULL`; формат `g_0001`/`p_0001`/`i_0001`/`e_0001` (номер растёт без обрезки); бэкфилл существующих записей по `id`; у `categories` метки нет (её `code` — семантический, §3.4); в студии — только чтение. Основа переноса контента (строка экспорта ниже) |
| Экспорт снимка контента каталога (спека `2026-09-24-каталог-экспорт-импорт-контента-на-прод` §4/§7, И2) | `GET /studio/api/content/export` (JWT admin/skycomposer) — снимок справочников студии по натуральным ключам + меткам `code` и без `id`. SQL — `internal/repository/content_export.go` (`ContentExport` — одна транзакция чтения REPEATABLE READ по `categories`/`goods`/`recipes`/`recipe_components`/`producer_types`/`items`/`producer_slots`/`producer_recipes`/`producer_items`/`effect_types` + ссылка-якорь `generation_config.default_settlement_type_id`); сборка файла — `internal/handlers/studio_content.go` (`ContentSnapshot`/`buildContentSnapshot`: секции топологически по FK, `producer_types` — родители раньше детей, `items.unlocks` резолвится из id-формы БД в метку/натуральный ключ `{producer, category{kind,name}}` (§4/T15); пустая метка контентной записи или нерезолвимая ссылка `unlocks` — ошибка). Файл `content/catalog.json` (env `CONTENT_CATALOG_PATH`, дефолт `content/catalog.json`) пишется best-effort и отдаётся на скачивание (`Content-Disposition: catalog-YYYYMMDD.json`); `content/` копируется в образ (`Dockerfile`). UI — кнопка `#btnExportContent`/`exportContent` в шапке `web/studio.html`; общий для экспорта/импорта формат снимка — пакет `internal/goodsstudio/contentio`. `producer_types`/`producer_slots` несут `category_kind` (имя категории неоднозначно между `good`/`resource`, напр. «топливо»). Импорт/полная замена — строка И3 ниже |
| Импорт снимка контента каталога (спека `2026-09-24-каталог-экспорт-импорт-контента-на-прод` §5/§7, И3) | `POST /studio/api/content/import` (JWT admin/skycomposer): тело `{}` (файл `content/catalog.json`) или `{"snapshot":{…}}`; `?dry_run=true` — сухой прогон (дифф без записи). `GET /studio/api/content/status` — наличие файла (кнопка импорта активна только при нём). Алгоритм — `internal/repository/content_import.go` (`ImportContent`): одна транзакция с `pg_advisory_xact_lock` (`beginMutation`), режим сопоставления **managed** (по метке `code`) или **bootstrap** (по `(kind,name_norm)` при цели без меток; смесь → `unmatched` + отказ), upsert полей без смены `id` (мировые FK целы), дочерние наборы (`recipe_components`/`producer_slots`/`producer_recipes`/`producer_items`) заменяются целиком для затронутой записи, полная замена (удаление отсутствующего в файле) с **предсчётом мировых ссылок** (`deposits`/`player_cargo`/`settlement_branch_buffers.good_id`, `settlement_branches.recipe_id`, `settlements.settlement_type_id`, `buildings.producer_type_id`, `active_effects.effect_type_id`) — любая ссылка → `blocked`, 409 и полный откат (§5.4); `default_settlement_type_id` резолвится по метке якоря. `dry_run` даёт дифф `{mode,create,update,delete,blocked,unmatched}`, применение — отчёт `{created,updated,deleted,warnings}`. Формат снимка — `internal/goodsstudio/contentio`; ручки — `internal/handlers/studio_content.go` (`ContentImport`/`ContentStatus`), роуты — `cmd/server/main.go`. UI — кнопка `#btnImportContent`, двухшаговый попап `#importPopup` (Проверить → Применить) в `web/studio.html` |
| Деньги и эскроу, фундамент (спека `2026-09-22-деньги-и-эскроу`, этап A) | Счёт актора + журнал движений: таблицы `accounts`/`money_operations` (миграция `000061`), `npc_agents.owner_faction_id` (владелец-фракция агента, §6). Модель — `internal/models/account.go` (`AccountOwnerPlayer/Faction/Agent`, `PlayerBalanceSeed=10000`, `FactionBalanceSeed=10^15`, `Account`/`MoneyOperation`). SQL — `internal/repository/account_repository.go` (`EnsureAccount` идемпотентно, `Debit`/`Credit` одним условным оператором — проверка+действие атомарно, `GetBalance`/`GetOperations`); регистрация в одной транзакции — `user_repository.go` `CreateWithAccount` (§3.4). HTTP — `internal/handlers/money_handlers.go` (`GET /me/money` — свой баланс + своя история, О-д4; `owner_id` из JWT, чужое не отдаётся), ленивая страховка счёта — `auth_handlers.go` (`/register` в той же tx, `/me` best-effort). Залог/эскроу (lock/release/return) — этап B; счёта поселений/построек нет (решение 11). UI-потребитель: `GET /me/money` → `web/static/js/dashboard/money.js` (строка «Счёт» в шапке дашборда/карты, вкладка «💳 Счёт», 2026-09-23) |
| Контракт как сущность + эскроу (спеки `2026-09-22-контракт-модель-сущности` §4/§6, `2026-09-22-деньги-и-эскроу` §3–§4, `2026-09-22-контракт-перелёт-и-доска` §5) | Таблицы `contracts`/`contract_requirements`/`contract_log` (миграция `000062`), `DROP TABLE assignments` (снос муляжа `internal/models/assignment.go`, `internal/models/contracts.go`, `internal/repository/assignment_repository.go`). Модель — `internal/models/contract.go` (статусы, типы автора/исполнителя, типы лога). SQL — `internal/repository/contract_repository.go`: `Publish` (одна tx: lock залога со счёта автора через CTE `FOR UPDATE` + `escrow_withdrawable = LEAST(withdrawable, amount)`, вставка контракта/требований, `contract_log` published+escrow_locked, `money_operations('escrow_lock')`; supply с `reward = 0` — бесплатная публикация без залога и движения, миграция `000088`), `Take`/`Cancel`/`Complete` — атомарные flip'ы `UPDATE ... WHERE status=$old` (0 строк = гонка; `Complete` дополнительно требует `expires_at > NOW()` — просроченный `taken` закрывает `ExpireDue`), `ExpireDue` (ленивое истечение §6.3), `ReturnEscrowForContractsTx` (атомарный статусный flip §6.5 + возврат залога; пустая `ContractScope{}` = вся таблица), `ResolvePublicationPlanet` (место публикации по автору §3: faction → `homeworld_id`, building → `planet_id`, settlement → `settlements.planet_id`; player резолвит хендлер по позиции, agent не поддержан), `resolvePayerAccountQ` (§4: player/faction напрямую, building → владелец, agent → фракция, settlement → владелец поселения, при владельце-агенте — счёт его фракции D4). HTTP — `internal/handlers/contract_handlers.go`: `GET /api/planets/{id}/contracts` (доска, гейт знанием планеты для player), `GET /api/contracts/mine`, `POST /api/contracts` (публикация игроком — только с планеты, О-п1), `POST /api/contracts/take|cancel`, `POST /admin/contracts` (остальные авторы + отладка; планета — по автору, не из тела). Возврат залога в путях удаления — `clearUniverseTx`/`clearPlanets`/`clearPlanetsOf`/`DeleteWorld`/пакман (`ReturnEscrowForContractsTx`); `accounts` не в `truncateTables`, счета фракций и агентов удаляются после TRUNCATE (§3.5). Фронт-вкладка доски и тип «перелёт» — B2/B3 |
...
| Мир прогулки — скульптурный объём, фундамент Э5.1 (спека `2026-09-25-мир-прогулки-скульптурный-объём`) | Единое поле твёрдости `solidAt` — единственный источник (база ⊕ формы) для физики и растра: `web/static/js/surface/surface_world.js` (`solidAt`/`baseSolid`/`formsSolid`/`columnSpans`/`skyTop`/`floorY`, `isSolid` — алиас; `caveAt(x,y,th)` — предвычисленный профиль для маски), растр — `surface_render.js` (`getChunkCanvas`: главный проход из `columnSpans`, консервативная пещерная маска — 3×3 только если весь блок воздух, граница шаг 1 px; сброс кэша по `SS` и `_viewKey`, вытеснение `_chunkCache` в `drawTerrain`), угловая коллизия бокса 12×28 — `surface_player.js`; бюджет вертикали сервера (+24) — `internal/generator/planet/biome_view.go`; проверки — `tools/surface-field-check.mjs`, `tools/e2e/surface-solid-field-check.js`, e2e `tools/e2e/surface-walk-field-check.js` |
...
| Ветка поселения (спека `2026-09-22-поселение-ветка-буферы-переработка`, итерация 2; добыча из залежи — итерация 3; **внутреннее хранилище — ЧК2а, подэтап 2б, 2026-09-25**) | Таблица `settlement_branches` (миграция `000064`); буферы ветки `settlement_branch_buffers` **ретайрены** (`000087`, RENAME в `_retired`) — физический запас в `settlement_storage_cells` (миграция `000086`), ядро расчётов `internal/economy/settlement/storage.go` (`StorageCaps`/`GoodWeights`/`AllocateProportional`/`CellDeficit`), доступ — `internal/repository/storage_cell_repository.go`. Переработка — `internal/economy/settlement/branch.go` (линейная скорость `k·population/complexity`, `k = 2.78·10⁻⁸`, `MinPersistInterval` 30 мин); добыча из `deposits` (лок ветка → залежи) — `internal/repository/branch_repository.go`; owner-проход (ячейки, F2/F4/F6) — `internal/repository/settlement_owner_pass.go`; HTTP — `internal/handlers/admin_settlement_branches.go`; модель — `internal/models/economy.go` (`SettlementBranch` без буферов, `StorageCell`); клиент — `web/static/js/modal/branches.js`, `modal/deposits.js`/`tabs.js` |
| Ориентация спрайта корабля (спека `2026-09-21-угол-корабля-в-метаданных`) | Пара `(A, F)` в метаданных вместо поворота пикселей: реестр `RaceShipSprites` (`ShipSprite.Angle`/`Flip`) — `internal/models/race_ship_sprites.go`; показ — `web/static/js/map/ship_sprites.js`/`map_render.js` (`rotate(H + V·A)`, `scale(F ? −1 : 1, V)`, `V = cos H < 0 ? −1 : 1`), дашборд; студия — CSS-трансформ; импорт принятых — `tools/import_ship_sprites.ps1` (sha256 + гвард `orient_meta`) |
...
| Пояса — добыча, сцена и стек слоёв интерфейса (2026-09-23) | Сцена добычи — страница `web/belt.html` + клиент `web/static/js/belt/` (`belt_world.js` — стриминг ячеек вдоль кольца и спад плотности, поля тела `sprite`/`veinPattern`/`veinRot`/`veinScale`/`veinRich`; `belt_render.js` — прицел и подсветка наведённой жилы, реестр спрайтов и прелоад; `belt_input.js` — ввод мышью «два стика»; `belt_config.js`/`belt_main.js`/`belt_ui.js`); ассеты сцены — `web/static/sprites/belt/` (16 форм камней × 2 текстуры, 3 мелких обломка, 4 паттерна руды); кольцо пояса на схеме системы — `web/static/js/modal/layout.js` (`beltMidRadius`/`beltRing`/`beltAngle`/`beltPoint`), `modal/modal_render.js` (`drawBelts`/`drawBeltStone`), ПКМ-меню `showBeltMenu` — `modal/events.js`; единый реестр слоёв интерфейса — `web/static/js/ui/layers.js` (полосы, маршрутизируемые/пассивные, Esc и клик-вне только верхнего, ловушка фокуса, `lockScroll`) + диалоги `web/static/js/ui/alert.js`; на реестр переведены тост (`ui/toast.js`), модалка системы (`modal/*`), меню/тултипы карты (`map/*`), поиск (`search.js`), попапы NPC и пакмана; e2e — `tools/e2e/belt-mining-check.js`, `belt-sprites-shot.js`, `belt-halo-check.js`, `belt-nearest-check.js`, `belt-stones-check.js`, `layers-check.js`, `qa-belt-null-enter.js` |
| Ледяные астероиды — второй ресурс пояса (спека `2026-09-24-ледяные-астероиды-вода`, расширение спеки этапа 3) | `internal/handlers/belt_mining_handlers.go` — второй ресурс: `collect{amount, resource}` (`iron`/`ice`), тройной кламп по выбранному ресурсу и **общий трюм** (`mined + mined_ice + granted ≤ free`); `internal/repository/planet_repo.go` (`SetBeltIceRemaining`, чтение `ice_remaining`); `BeltView`/`models.Belt` (+`IceRemaining`/`IceClass`/`RemainingLevelIce`); клиент `web/static/js/belt/*` (два ресурса); арт `web/static/sprites/belt/ice_*`, пайплайн `tools/make_belt_*` + `tools/batch_belt_ice.ps1`; e2e `tools/e2e/{belt-mining-check,belt-ice-shot,belt-sprites-shot}.js` |
| Магазин модулей — витрина и покупка планеты (спека `2026-09-24-магазин-модулей-локальный-рынок`, подэтапы C1/C2) | Таблица `market_offers` (миграция `000081`, сид 4 базовых модулей по 3000 Cr, каталог-контент). Модель — `internal/models/market_offer.go` (`MarketOffer`, `Item *EquipmentItem` — разворот в хендлере). SQL — `internal/repository/market_repository.go` (`ListOffers`/`GetOfferByID`/`HasLiveSettlement`). HTTP — `internal/handlers/market_handlers.go`: `GET /api/planets/{id}/market` (§7.1: гейт знанием — none → 404, `settlements_count == 0` → `offers: []`; `can_trade` — присутствие игрока (роль player) на орбите планеты, спутник — по родителю; прецедент `can_buy_report`) и `POST /api/planets/{id}/market/buy` (§7.2: одна транзакция — лок строки игрока `FOR UPDATE`, трейд-ин `floor(price/2)` из `market_offers`, списание одним `UPDATE` с `withdrawable = LEAST(...)`, установка `jsonb_set`, журнал `money_operations kind='purchase'`; деньги сгорают — печь §8). Дельта чтения модулей §9 (И-М5): `RadarRadius`/`HasScanner`/`HasEngine` (`internal/ship/radar.go`/`catalog.go`) читают модуль по типу из ЛЮБОГО слота, радар — максимум (не сумма); `EngineSpeed` v1 — по-прежнему из легаси-слота `engine`. Диспетчер `/api/planets/` — `cmd/server/main.go`. Интеграционные тесты (живая БД) — `internal/integration/market_test.go` |
| GDD | `docs/gamedesign/` |
...
├── modal/                 — модалка системы (tabs, panel, index, state, events, layout, modal_render — кольцо пояса)
├── belt/                  — мини-игра добычи в поясе (стриминг ячеек, рендер, ввод мышью; два ресурса — железо и лёд, холодные спрайты ice_*)
├── ui/                    — стек слоёв интерфейса (layers.js) и диалоги (alert.js)
└── map/
