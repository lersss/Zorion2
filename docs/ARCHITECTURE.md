...
| Картинка планеты (спека 2026-09-20) | честный генератор — `internal/generator/planet/planet_image_v2.go` (один генератор на типоразмеры: поверхность из биомов {form,share} → пятна с шум-границами, контракт «биом → цвет» — база категории + сдвиги + опциональный `color` в каталоге; атмосферный слой дымка/лимб/свечение/облака; заслонение cloudCover из видимых параметров — атмосферные числа НЕ входят, И1; заглушка — нейтральный силуэт по seed; кэш-ключ hash64(planet_id)|mode|size, in-memory 1000 FIFO); старый косметический генератор — `planet_image.go` (фолбэк старых миров без биомов); эндпоинт — `internal/handlers/planet_image_handler.go` (авторизованный `/api/planet-image?planet_id&size`, режим честная/заглушка по роли/своей системе/знанию), диск-кэш большой — `internal/handlers/planet_image_disk_cache.go` (PNG 512 в `{PLANET_IMAGE_CACHE_DIR}/{sha256(planet_id|mode)}.png`, лимит 1000 LRU по mtime, битый файл → перегенерация); клиент — `web/static/js/modal/textures.js` (fetch+blob, JWT, ключ id|size|own/foreign), `tabs.js` (большая «Вид с орбиты» при orbit {planet} или админ, иконки биомов PNG 22px + onerror-фолбэк); иконки — `web/static/sprites/biomes/*.png` (23 шт, пачка 1) |
