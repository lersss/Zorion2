...
|---|---|---|
| `internal/goodsstudio/seed_producers.go` (+`seed_producers_test.go`), `internal/goodsstudio/settlement_type_consistency_test.go`, `internal/goodsstudio/supply_effects_consistency_test.go`, `migrations/000077_settlement_stages_ladder.sql` (**бронь номера миграции `000077`**) | @manager (задача «ступени поселения»: 7 подтипов-ступеней «Аутпост → Посёлок → Городок → Город → Мегаполис → Метрополия → Экуменополис» с порогами; базовая ступень = переименование `148`) | до коммита задачи |
| `web/studio.html`, `web/static/js/modal/branches.js`, `internal/handlers/planet_visibility.go` | @developer (задача «переключатель масштаба единицы»: отображение/ввод «ед/сутки» в масштабах 1 чел / 1000 / 10⁶ / 10⁹ в студии и витрине; модель не меняется) | до коммита задачи |
