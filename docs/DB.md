...
- `000082` — `000082_structures_owner.sql` — постройка структур на планете из
  ...
  **Бэкфилла владельцев нет** (решение создателя Г1): существующие поселения
  ...
  после `000081`.
- `000083` — `000083_settlement_rename.sql` — ренейм-миграция канон-имён класса
  поселения (спека `2026-09-24-каталог-экспорт-импорт-контента-на-прод` §6,
  итерация И1): класс-корень «Поселение» → «Колония», ступени «Аутпост» →
  «Форпост», «Посёлок» → «Поселение». Резолв — **по данным** (корень = родитель
  ступени ладдеры `params ? 'stage'` либо родитель типа-якоря
  `generation_config.default_settlement_type_id`; ступени — по `parent_id`
  корня), **не по литералу-двойнику** имени `поселение` (до ренейма его носят
  класс-корень и вторая ступень). Порядок «корень → ступени» из-за
  `UNIQUE(name_norm)`; идемпотентно (уже переименованная БД / нет старого имени
  — no-op). `default_settlement_type_id` и `settlements.settlement_type_id` —
  id-ключевые, **не трогаются**. Коллизия целевого имени (`колония` занято иным
  типом) — явный `RAISE` (молчаливой порчи нет). На dev-БД переименование уже
  сделано вручную — миграция no-op (проверено). Номер `000083` зафиксирован
  менеджером.
- `000084` — `000084_catalog_code.sql` — номерная метка записи каталога (метка
  переноса `code`, спека `2026-09-24-каталог-экспорт-импорт-контента-на-прод`
  §3.1–3.3, итерация И1): у `goods`/`producer_types`/`items`/`effect_types` —
  колонка `code TEXT NULL` + `SEQUENCE`-счётчик на таблицу
  (`<table>_code_seq`), DEFAULT `catalog_next_code('<префикс>', '<seq>')` и
  частичный UNIQUE `(code) WHERE code IS NOT NULL` (уникальность в пределах
  таблицы). Префиксы `g_`/`p_`/`i_`/`e_`, номер минимум 4 цифры, растёт без
  обрезки (`g_10001`; `lpad` обрезает, `to_char` переполняется — `docs/pitfalls/db-shell.md`).
  Бэкфилл существующих — по порядку `id`, только `NULL` (идемпотентно), счётчик
  сдвигается на максимум. `categories` метки не получает (у неё семантический
  `code`, §3.4). Новая запись (сид/студия/ИИ) берёт метку из счётчика
  автоматически (DEFAULT) — отдельной правки сида не требуется. Номер `000084`
  зафиксирован менеджером (после `000083`).
- `000085` — `000085_producer_sections.sql` — разделы построек по видам (спека
  `2026-09-25-студия-разделы-построек-по-видам`, коммит `2b32533`): у
  `producer_types` — колонка `section TEXT NULL` (без `CHECK`; значения
