...
- **Фон: magenta (сцен-негатив) — рецепт обновлён 2026-09-22.** Новые генерации
  пишут фон **magenta** (`ShipBackground`/`shipNegScene`), а не чёрный. Вырез —
  `--method edge` (flood от рамки + барьер по Собелю). Порог зависит от фона:
  magenta-кадры — `--tol 100 --edge 20`, чёрно-фоновые — `--tol 40 --edge 20`;
  авто-детект фона — `is_magenta_bg` в `tools/ship_recut_pool.py` (студийный
  `postproc/ship.go` — 100). `hyst` и `chroma` оставлены как откат. Хромакей по
  magenta (`--method chroma`) **отклонён**: SDXL даёт градиент, а не ровную
  заливку. `rembg` не используем (режет корабль — «кольцо от станции»).
