...
> Компактный обзор docs/impact_map.json (73 сущности, ~520 связей, ~162 КБ — целиком НЕ читать). Вопрос реестра: «меняем X → смотрим на Y». Ведёт @manager при сдаче фич (идея 83a); валидатор — scripts/check_impact_map.ps1 после каждого коммита.
...
| belt | internal/generator/planet/planet_data_belt.go | пояс малых тел — объект мира и агрегат (2026-09-22, коммит b24e358): астероидный (орбита g−1, резонансное окно гиганта 3:1–2:1), Койпера (за x_out=35), debris (WD); бюджет облака — общая масса (w_i=c_i/S₀, приор median=S₀, шум ζ не нормируется), мягкий кламп суммы на уровне мира (~17% систем); игроку — этапы 2 (показ/полёт) и 3 (добыча) |
| system_belts | db:system_belts | таблица поясов (000059): world_id FK worlds CASCADE, kind — свободный ключ (asteroid/kuiper/oort/debris/dust_ring), radius_au/width_au/mass/body_size_km, composition/data JSONB; агрегат (поля bodies нет); в truncateTables/clearPlanets |
