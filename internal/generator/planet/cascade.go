...
const (
	...
	// Границы массы каменистых/ледяных планет: пол — страховка реестра
	// (генератором не достигается); massMax — критическая масса ядра
	// M_crit (порог убегающей аккреции газа, спека
	// 2026-09-23-перелив-массы-в-гигантов-и-мини-нептуны §5/§7.1): не кламп,
	// а граница ветки оболочки (тела > M_crit идут в мини-нептунов/гигантов).
	// Коридор калибровки M_crit ∈ [5, 10], жёстко < 16.
	massMin = 0.02 // M⊕ — пол (страховка реестра)
	massMax = 8.0  // M⊕ — M_crit (верх каменистой ветки)
...
type cascadeInput struct {
	...
	// spec 2026-09-23): > 0 — масса уже посчитана миром (с f_обр или веткой
	// оболочки), каскад её не роллит и не клампит. 0 — легаси-путь
	// (экзотика/прототип/прямые вызовы): ζ ролится здесь, как раньше.
	OverflowMass float64 // M⊕
...
func (g *Generator) runCascade(in cascadeInput) *cascadeResult {
	...
	if mass <= 0 {
		...
		if in.OverflowMass > 0 {
			// Массу посчитал пред-слой перелива (§4.2 проход 5): f_обр уже
			// применён после фиксации g, ζ и кламп — там же. Роллов нет.
			mass = in.OverflowMass
		} else {
