...
// v8 (спека 2026-09-23 §4.7.5): заполнение color у 7 биомов вулканизма (ЧК3)
// меняет и картинку планеты с орбиты — без бампа дисковый кэш
// (planet|mode|size|v7) показал бы старый цвет («орбита старая, прогулка новая»).
// v7 — тот же бамп для 11 биомов биосферы (ЧК2, §4.6.5); v6 — 10 биомов
// литосферы (ЧК1, §4.5.5); v5 — 4 образца (§2.5).
const ImageGenVersion = "v8"
