...
// v9 (спека 2026-09-23 §4.8.8): заполнение color у 5 биомов крио (ЧК4)
// меняет и картинку планеты с орбиты — без бампа дисковый кэш
// (planet|mode|size|v8) показал бы старый цвет («орбита старая, прогулка новая»).
// v8 — тот же бамп для 7 биомов вулканизма (ЧК3, §4.7.5); v7 — 11 биомов
// биосферы (ЧК2, §4.6.5); v6 — 10 биомов литосферы (ЧК1, §4.5.5); v5 — 4 образца (§2.5).
const ImageGenVersion = "v9"
