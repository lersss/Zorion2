...
func worldRow(id, name string, now time.Time) []driver.Value {
	return []driver.Value{id, name, 0.0, 0.0, "G", 5778, "star", "single", nil, nil, now, now}
}
...
func TestGetAllWorldsWithGalaxyPopulation(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT id, name, coord_x, coord_y, COALESCE(spectral_class,''), temperature, star_type, system_type, stellar_mods, stellar_mass, created_at, updated_at FROM worlds ORDER BY name LIMIT $1 OFFSET $2`).
		WithArgs(50, 0).
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "coord_x", "coord_y", "spectral_class", "temperature", "star_type", "system_type", "stellar_mods", "stellar_mass", "created_at", "updated_at"}).
...
func TestGetAllWorldsSortByPopulationDesc(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT id, name, coord_x, coord_y, COALESCE(spectral_class,''), temperature, star_type, system_type, stellar_mods, stellar_mass, created_at, updated_at FROM worlds ORDER BY name`).
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "coord_x", "coord_y", "spectral_class", "temperature", "star_type", "system_type", "stellar_mods", "stellar_mass", "created_at", "updated_at"}).
			AddRow(worldRow("w1", "Альфа", now)...).
			AddRow(worldRow("w2", "Бета", now)...))
