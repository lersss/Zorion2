...
// Направления тренда (вкладка «Миры»). Рост в модели пока не даёт ни одна
// компонента (r ≥ 0, change_components.go), но ветка заготовлена: тренд
// считается честно от мгновенного изменения, а не от истории (истории нет).
const (
