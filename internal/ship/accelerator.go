...
// ПРИМЕЧАНИЕ (§14.14 спеки ускорителя — развилка создателя): в модели v9 награду
// задаёт кривая поля, поэтому bonus_max/AcceleratorBonus в v9-пути не читаются.
// Оставлены намеренно — это ось прогрессии модуля (accel_1 → accel_2 → accel_3),
// решение по ней за создателем; до решения не удалять.
//
