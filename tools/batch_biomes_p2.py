# -*- coding: utf-8 -*-
# Батч-раннер иконок биомов пачки 2 (34 недостающих): этап 1 (форма, ControlNet Canny) -> этап 2 (детализация, img2img).
# Выход: ai_drafts/biomes/batch_p2B/<num>.png. Продолжение: --start N --count M. Лог: batch_p2B/log.txt.
import argparse, json, os, shutil, time, urllib.request
from biome_manifest_p2 import ITEMS

COMFY = 'http://127.0.0.1:8188'
COMFY_INPUT = r'C:\ComfyUI\input'
OUT_DIR = r'C:\Zorion2\ai_drafts\biomes\batch_p2B'
...
def workflow_controlnet(prompt, neg, sil_path, seed):
    ...
    return {
        ...
        '7': {'class_type': 'SaveImage', 'inputs': {'images': ['6', 0], 'filename_prefix': 'zorion_biome2'}},
    }
...
def main():
    ...
    for i in range(args.start - 1, end):
        ...
        try:
            ...
            pid2 = post_prompt(workflow_img2img(STAGE2_NORMAL, NEG2, s1_path, seed + 500))
            img2 = wait_result(pid2)
            download(img2, fin_path)
            log('  ok %s (%d bytes)' % (item['id'], os.path.getsize(fin_path)))
        except Exception as e:
            log('  FAIL %s: %s' % (item['id'], e))
    log('=== batch p2 done (start=%d count=%d) ===' % (args.start, args.count))
