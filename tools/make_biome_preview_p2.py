# -*- coding: utf-8 -*-
# Пост-обработка пачки 2 иконок биомов (стиль B): process_icon.py -> 48x48, id биома = имя файла.
# + preview_p2.html (сетка с номерами 1..34, сравнение 48/64, демо 24px в строке текста).
import os, subprocess, sys
sys.stdout.reconfigure(encoding='utf-8', errors='replace')
from biome_manifest_p2 import ITEMS

PY = r'C:\ComfyUI\venv\Scripts\python.exe'
PROC = r'C:\Zorion2\tools\process_icon.py'
BATCH = r'C:\Zorion2\ai_drafts\biomes\batch_p2B'
OUT = r'C:\Zorion2\ai_drafts\biomes\processed_p2B'
...
for i, item in enumerate(ITEMS):
    num = i + 1
    cards.append("<div class='card'><div class='num'>%d</div><img src='processed_p2B/%s.png'><div class='name'>%s</div></div>" % (num, item['id'], item['name']))

demo_biomes = ['горы', 'пески_пустыни', 'кратеры', 'озёра_реки', 'ледники', 'джунгли', 'радиационные_ковры', 'пещерный_мир_с_потолком']
demo_rows = []
for bid in demo_biomes:
    name = next(x['name'] for x in ITEMS if x['id'] == bid)
    demo_rows.append("<div class='demorow'><img src='processed_p2B/%s.png'><span>%s</span></div>" % (bid, name))
demo_block = "<h2>Иконка ~24px в строке текста (рядом с именем биома)</h2><div class='demowrap'>%s</div>" % ''.join(demo_rows)

html = """<!DOCTYPE html><html><head><meta charset='utf-8'><title>Zorion — иконки биомов (пачка 2, стиль B)</title>
...
<h1>Zorion — иконки биомов поверхности, пачка 2 — стиль B (мини-пейзаж), %d шт</h1>
<div class='legend'>Добивка 34 недостающих биомов (пачка 1 — 23 шт уже приняты). Небо чистое — без солнц/лун/звёзд/планет. Финальный размер — 48×48; вид 24px в тексте — ниже.</div>
<div class='grid'>%s</div>
%s
</body></html>""" % (len(ITEMS), ''.join(cards), demo_block)

with open(os.path.join(r'C:\Zorion2\ai_drafts\biomes', 'preview_p2.html'), 'w', encoding='utf-8') as f:
    f.write(html)
