...
// место клика по кольцу (проекция на осевую линию — «летим туда, куда кликнули»),
// а без точки клика (ПКМ по строке списка) — ближайшая точка осевой линии к
// кораблю на момент старта полёта («не лететь через полкарты к чужой точке
// кольца»). Живёт в модуле — переживает закрытие/переоткрытие модалки (иначе
// маркер «я здесь» прыгал бы на канонический азимут); после перезагрузки
// страницы (F5) карта пуста — beltPoint берёт канонический beltAngle, и маркер
// не исчезает (§9.3 п.2).
...
// на осевой линии). Точка — место клика по кольцу (правка 2026-09-23) либо
// позиция корабля (фолбэк) — формула одна. Совпадает с центром (dx=dy=0) — null
// (нет направления).
...
// beltPoint — точка пояса (середина кольца, осевая линия): место клика по кольцу
// (beltArrivalAngles), иначе каноническая (beltAngle).
